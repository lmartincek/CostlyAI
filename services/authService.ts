...
export const signInWithPassword = async (email: string, password: string) => {
  try {
    ...
    if (error)
      return returnError(`Failed to login with credentials: ${error.message}`, error.status)
    return data
  } catch (error: any) {
...
export const signInWithOAuth = async (provider: Providers) => {
  try {
    ...
    if (error)
      return returnError(`Failed to login with ${provider}: ${error.message}`, error.status)
    return { url: data.url }
  } catch (error: any) {
