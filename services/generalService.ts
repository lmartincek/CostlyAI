...
export const fetchRecentlySearchedPlaces = async (
  places: number = 4,
): Promise<Place[] | FailedResponse> => {
  try {
    const { data, error } = await supabase
      .from('products')
      .select(
...
export const addUserSearch = async ({
  ...
  try {
    const { data, error } = await supabase.from('user_searches').insert([payload]).select('id')

    if (error)
      return returnError(`Failed to insert user search: ${error.message}, hint: ${error.hint}`, 400)
    return data
  } catch (error: any) {
...
export const fetchUserSearch = async (
  userId: string,
  places: number = 4,
): Promise<UserSearch[] | FailedResponse> => {
  try {
    const { data, error } = await supabase
      .from('user_searches')
      .select(
