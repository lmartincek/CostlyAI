...
export const getChatResponse = async (req: Request, res: Response) => {
  ...
  const aiProducts: ProductAIResponse[] | FailedResponse = await fetchChatCompletion(
    countryName,
    cityName,
    selectedCategories,
  )
  if ('error' in aiProducts) {
    ...
    return res
      ...
      .json(returnError(`Failed to get response from openAI controller: ${error}`))
  }
...
export const getChatStreamResponse = async (req: Request, res: Response) => {
  ...
  if ('error' in stream) {
    ...
    return res
      .status(statusCode ?? 500)
      .json(returnError(`Failed to get response from OpenAI streaming: ${error}`, statusCode))
  }
