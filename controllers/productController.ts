...
import { addProducts, addUserProducts, fetchProducts, fetchUserProducts } from "../services/productService";
...
export const saveUserProducts = async (req: Request, res: Response) => {
    ...
    if (!userId || !userSearchId) {
        return res.status(400).json(returnError(`Missing required userId: ${userId}, or userSearchId: ${userSearchId}`, 400));
    }
